...
- **국내 테크 기업 / 국내 테크 기업** 서버 개발 공고의 기술스택이 그대로 **"Node.js, TypeScript, Nest.js, Jest"**다. 오늘 배우는 `@Module`/`@Controller`는 그 공고에서 요구하는 프레임워크의 가장 기본 골격이다.
- **국내 테크 기업**도 백엔드 표준으로 NestJS를 채택했다. 실무 팀은 대부분 "모듈 단위로 기능을 쪼개는" 구조를 쓰기 때문에, `@Module`을 읽고 앱 구조를 파악하는 능력이 온보딩 첫날부터 필요하다.

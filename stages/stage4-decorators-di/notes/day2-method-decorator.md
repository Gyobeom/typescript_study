...
5. 이게 AOP(관점 지향 프로그래밍)의 축소판이며 NestJS AOP 라이브러리들의 원리임을 안다.
...
- **NestJS AOP 라이브러리**: 커뮤니티에는 메서드 데코레이터로 트랜잭션·로깅 같은 관점을 붙이는 AOP 라이브러리들이 있다. 오늘의 `@LogExecution`이 그 축소판이다. 원리를 알면 그런 라이브러리 코드가 "번역 없이" 읽힌다.

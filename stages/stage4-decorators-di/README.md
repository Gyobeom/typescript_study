...
- **NestJS Custom Decorator를 다루는 실무 기술 아티클들**은 정확히 이 스테이지 수준의 지식(클래스/메서드/파라미터 데코레이터, 메타데이터, IoC)을 **전제**로 읽힌다. 이 스테이지를 마치면 그런 글이 "번역 없이" 읽힌다.
- 커뮤니티의 **NestJS AOP 라이브러리들**은 메서드 데코레이터로 관점(로깅·트랜잭션·캐싱)을 주입하는 패턴을 표준화한 것이다. Day 2의 `@LogExecution`이 그 축소판이다.

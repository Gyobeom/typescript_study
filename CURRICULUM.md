...
본 문제는 힌트가 많아 "이해"까지는 데려다주지만, **힌트 없이 스스로 꺼내 쓰는 훈련**은 별도로 필요하다. 각 스테이지에는 힌트 제로·새 도메인의 **종합 복습 문제**(`exercises/review-*.ts`)가 있다.

- **시점**: 다음 스테이지 시작 **전에** 이전 스테이지 복습 문제를 푼다 (예: 스테이지 4 시작 전에 `npm run review:stage3`). 하루 이상 간격을 두면 효과가 더 크다.
- **규칙**: 노트·본 문제·solutions를 **보지 않고** 푼다. 막히면 그게 바로 복습이 필요한 지점 — 노트를 다시 읽고, 덮고, 다시 시도.
- **추가 인출 연습**: 각 노트 끝의 "셀프 체크" 질문에 노트 안 보고 소리 내어 답해본다. 설명이 막히면 아직 내 것이 아니다.
- 채점: `npm run review:stageN` (복습은 `check:stageN` 채점과 분리되어 있어 완료 판정에 영향 없음)

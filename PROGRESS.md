# 학습 진행 트래커 (대시보드)

> 이 파일은 **전체 진도를 한눈에 보는 체크 대시보드**입니다.
> 그날의 상세 기록(배운 것, 막힌 점)은 `journal/YYYY-MM-DD-stageN-dayM.md`에 일자별 파일로 남기세요 (`journal/_TEMPLATE.md` 복사).
> 스테이지 완료 판정: `npm run check:stageN` 전체 통과.

## 스테이지 1 — 클래스 기초 (1주차)

- [x] Day 1: 클래스 선언·생성자 — [일지](journal/2026-07-29-stage1-day1.md)
- [x] Day 2: 접근제어자 — [일지](journal/2026-07-29-stage1-day2.md)
- [x] Day 3: 파라미터 프로퍼티·getter/setter — [일지](journal/2026-07-29-stage1-day3.md)
- [x] Day 4: static — [일지](journal/2026-07-29-stage1-day4.md)
- [x] Day 5: 종합 문제 — [일지](journal/2026-07-30-stage1-day5.md)
- [x] **스테이지 완료** (`npm run check:stage1` 37/37 통과 — 2026-07-30)
- [ ] 복습: `npm run review:stage1` 통과 (힌트 제로 종합 문제)

## 스테이지 2 — 계약 설계 (2주차)

- [x] Day 1: interface / implements — [일지](journal/2026-07-30-stage2-day1.md)
- [x] Day 2: 추상 클래스 — [일지](journal/2026-07-30-stage2-day2.md)
- [x] Day 3: 상속 vs 조합 — [일지](journal/2026-07-31-stage2-day3.md)
- [x] Day 4: SOLID — [일지](journal/2026-07-31-stage2-day4.md)
- [x] Day 5: 종합 문제 — [일지](journal/2026-07-31-stage2-day5.md)
- [x] **스테이지 완료** (`npm run check:stage2` 25/25 통과 — 2026-07-31)
- [ ] 복습: `npm run review:stage2` 통과 (힌트 제로 종합 문제)

## 스테이지 3 — 제네릭 클래스 (3주차)

- [x] Day 1: 제네릭 기초 — [일지](journal/2026-08-03-stage3-day1.md)
- [x] Day 2: 제네릭 제약 — [일지](journal/2026-08-03-stage3-day2.md)
- [x] Day 3: 제네릭 + 인터페이스 — [일지](journal/2026-08-03-stage3-day3.md)
- [x] Day 4: Repository&lt;T&gt; — [일지](journal/2026-08-03-stage3-day4.md)
- [x] Day 5: 종합 문제 — [일지](journal/2026-08-03-stage3-day5.md)
- [x] **스테이지 완료** (`npm run check:stage3` 32/32 통과 — 2026-08-03)
- [ ] 복습: `npm run review:stage3` 통과 (힌트 제로 종합 문제)

## 스테이지 4 — 데코레이터와 DI (4주차)

- [x] Day 1: 클래스 데코레이터 — [일지](journal/2026-08-05-stage4-day1.md)
- [ ] Day 2: 메서드·프로퍼티 데코레이터 —
- [ ] Day 3: reflect-metadata —
- [ ] Day 4~5: 미니 DI 컨테이너 —
- [ ] **스테이지 완료** (`npm run check:stage4` 통과 확인)
- [ ] 복습: `npm run review:stage4` 통과 (힌트 제로 종합 문제)

## 스테이지 5 — NestJS 미니 프로젝트 (5주차)

- [ ] Day 1: Module / Controller —
- [ ] Day 2: Service와 DI —
- [ ] Day 3: Repository·DTO —
- [ ] Day 4: 예외 처리 —
- [ ] Day 5: e2e + 부팅 —
- [ ] **스테이지 완료** (`npm run check:stage5` 통과 + 부팅 확인)
- [ ] 복습: `npm run review:stage5` 통과 (힌트 제로 종합 문제)
